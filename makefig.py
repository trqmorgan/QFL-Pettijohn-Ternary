...
def field_boundaries(scheme):
    if scheme == 'Pettijohn_1977':
        ...
        c1 = ['outline only', (0,0), (0.5,1), (1,0), (0,0)]
        classifications = [c1]
        labels = []
...
def plot_qfl(data, top, left, right, matrix, plottype, toplab, leftlab, rightlab, color, size,):
    ...
    if plottype not in list_valid_types:
        raise ValueError("Plot type not recognised, valid types are Pettijohn_1977, Dickinson_1983 and blank")
        ...
        for i in range(len(classifications)):
            ...
            for j in range(len(index)):
                if index[j] or index1[j]:
                    final_data.loc[j, plottype] = classifications[i][
...
if __name__ == "__main__":
    ...
    # plot type options are 'Dickinson_1983' or 'Pettijohn_1977' or 'blank'
    # ToDo add more plot types, add grid option?
